/*
	...
	Adds checksum check for each getReading and if fails, returns false with bIsSync set to true
...
ScanPacket Sweep::getReading(bool &success)
{
    ...
    if (_readResponseScanPacket())
    {
		...
		// evaluate checksum of data block
		int calculatedSum = 0;
		for (int i = 0; i < 6; i++) {
			//add each status byte to the running sum
			calculatedSum += _responseScanPacket[i];
		}
		calculatedSum = calculatedSum % 255;
		// compare to received checksum byte
		if (calculatedSum==_responseScanPacket[6]) {
			success = true;
			return ScanPacket(bIsSync, rawAngle, distance, signalStrength);
		} else return ScanPacket(true, 0, 0, 0);	//if checksum fails, return false, set bIsSync to true and other values to 0
